  ...
  return (
    ...
    >
      {'Get In Touch'.split('').map((char, idx) => (
        <span className="hoverText" key={`${char}-${idx}`}>
          {char}
        </span>
      ))}
    </motion.h2>
...
function FormInput({ id, label, type, autoComplete, props = {}, error }) {
  const sharedProps = {
    ...
    className:
      'w-full rounded-2xl border border-black/15 bg-white/70 px-4 py-3 text-black placeholder-black/50 backdrop-blur-sm transition-all duration-300 focus:border-black focus:bg-white focus:outline-none focus:ring-2 focus:ring-black/20',
    ...props,
  };
