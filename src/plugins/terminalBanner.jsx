...
function terminalBannerPlugin(options = {}) {
  ...
  const getGitInfo = () => cacheManager.get('git', () => ({
  ...
  const getProjectSize = (dir, excludeDirs = []) => {
    try {
      ...
      logManager.log('error', `Error calculating project size: ${error.message}`);
  ...
  const getNodeModulesInfo = () => cacheManager.get('nodeModules', () => {
    try {
      const nodeModulesPath = path.join(process.cwd(), 'node_modules');
      if (!fs.existsSync(nodeModulesPath)) return { size: 'N/A', packages: 0 };
  ...
  const countFilesByType = (dir, extensions, excludeDirs) => {
    ...
    traverse(dir);
    return { counts, totalFiles: Object.values(counts).reduce((a, b) => a + b, 0), totalLines };
  };
  ...
  const getSecurityAudit = () => cacheManager.get('security', () => {
    try {
  ...
  const createLine = (text, width, border, color = chalk.white) => {
    ...
    return border.side + color(text) + ' '.repeat(Math.max(0, padding)) + border.side;
  };
  ...
  return {
    ...
    configureServer(server) {
      ...
      const fileStats = countFilesByType(path.join(process.cwd(), 'src'), includeExtensions, excludeDirs);
      ...
      if (customBanner) {
        ...
        // ASCII Art Header
        console.log();
        asciiLogos.vite.forEach(line => console.log(colors.primary.bold(line)));
        console.log();
      }
      ...
      console.log(createLine(
        `    🚀  ${chalk.bgBlue.white.bold(' VITE DEV SERVER ACTIVE ')}  🚀`.padStart(50),
        bannerWidth,
        border,
        colors.primary
      ));
      ...
      printSection('PROJECT INFORMATION', '📦', chalk.bgCyan.black.bold, () => {
        console.log(colors.success.bold('  📂 Name:        '), chalk.white(projectName));
        console.log(colors.success.bold('  📌 Version:     '), chalk.white(pkg?.version || 'N/A'));
        console.log(colors.success.bold('  👤 Author:      '), chalk.white(pkg?.author || 'N/A'));
        console.log(colors.success.bold('  📄 License:     '), chalk.white(pkg?.license || 'N/A'));
        console.log(colors.success.bold('  📝 Description: '), chalk.white(pkg?.description || 'No description'));
        if (pkg?.keywords?.length) {
          console.log(colors.success.bold('  🏷️  Keywords:    '), chalk.gray(pkg.keywords.join(', ')));
        }
      });

      // Server URLs
      printSection('SERVER URLS', '🌐', chalk.bgBlue.black.bold, () => {
        console.log(colors.secondary.bold('  🏠 Local:       '), colors.primary.underline(url));
        networkAddresses.forEach((addr, i) => {
          const label = i === 0 ? '  🔗 Network:     ' : '                  ';
          const networkUrl = `${protocol}://${addr.address}:${port}/`;
          console.log(colors.secondary.bold(label), colors.primary.underline(networkUrl));
          if (i === 0) console.log(colors.secondary.bold('                  '), chalk.gray(`(${addr.name})`));
        });
        if (showPortInfo) {
          const portStatus = checkPortAvailability(port) ? '✓ Available' : '⚠ In Use';
          console.log(colors.secondary.bold('  🔌 Port Status: '), chalk.white(portStatus));
        }
      });

      // Environment & System
      if (showEnvironment || showSystemInfo) {
        printSection('ENVIRONMENT & SYSTEM', '⚙️', chalk.bgYellow.black.bold, () => {
          if (showEnvironment) {
            console.log(colors.warning.bold('  ⚡ Mode:         '), chalk.white(server.config.mode.toUpperCase()));
            console.log(colors.warning.bold('  🔧 Node:         '), chalk.white(process.version));
            console.log(colors.warning.bold('  📦 NPM:          '), chalk.white(safeExec('npm -v')));
            console.log(colors.warning.bold('  🏗️  Vite:         '), chalk.white(`v${server.config.viteVersion || 'N/A'}`));
          }
          
          if (showSystemInfo) {
            console.log(colors.warning.bold('  💻 Platform:     '), chalk.white(`${os.platform()} (${os.arch()})`));
            console.log(colors.warning.bold('  🖥️  Hostname:     '), chalk.white(os.hostname()));
            console.log(colors.warning.bold('  👤 User:         '), chalk.white(os.userInfo().username));
            console.log(colors.warning.bold('  🧠 Memory:       '), 
              chalk.white(`${formatBytes(os.freemem())} free / ${formatBytes(os.totalmem())} total`));
            console.log(colors.warning.bold('  🔥 CPU:          '), 
              chalk.white(`${os.cpus()[0].model.substring(0, 45)}... (${os.cpus().length} cores)`));
            console.log(colors.warning.bold('  ⏱️  Uptime:       '), chalk.white(formatDuration(os.uptime() * 1000)));
          }
          
          if (showTimestamp) {
            console.log(colors.info.bold('  🕒 Started:      '), chalk.white(new Date().toLocaleString()));
          }
        });
      }
      ...
      if (showGit && gitInfo && gitInfo.branch !== 'N/A') {
        printSection('GIT REPOSITORY', '🔀', chalk.bgMagenta.black.bold, () => {
          console.log(colors.info.bold('  🌿 Branch:       '), chalk.white(gitInfo.branch), 
            gitInfo.isDirty ? chalk.red(' (uncommitted changes)') : chalk.green(' (clean)'));
          console.log(colors.info.bold('  📍 Commit:       '), chalk.white(gitInfo.commit));
          console.log(colors.info.bold('  💬 Message:      '), chalk.gray(gitInfo.message.substring(0, 50)));
          console.log(colors.info.bold('  👤 Author:       '), chalk.white(`${gitInfo.author} <${gitInfo.email}>`));
          console.log(colors.info.bold('  📅 Last Commit:  '), chalk.white(gitInfo.date));
          console.log(colors.info.bold('  🔗 Remote:       '), chalk.cyan(gitInfo.remote));
          console.log(colors.info.bold('  📊 Total Commits:'), chalk.white(gitInfo.totalCommits));
        });
      }

      // Storage & Statistics
      if (showStorage || showProjectStats) {
        printSection('STORAGE & STATISTICS', '💾', chalk.bgGreen.black.bold, () => {
          if (showStorage) {
            console.log(colors.success.bold('  📊 Project Size:       '), 
              chalk.white(`${formatBytes(projectInfo.size)} (${projectInfo.files.toLocaleString()} files)`));
            console.log(colors.success.bold('  📦 node_modules:       '), 
              chalk.white(`${nodeModulesInfo.size} (${nodeModulesInfo.packages} packages)`));
          }
          
          if (showProjectStats) {
            console.log(colors.success.bold('  📄 Source Files:       '), chalk.white(fileStats.totalFiles));
            console.log(colors.success.bold('  📝 Total Lines:        '), chalk.white(fileStats.totalLines.toLocaleString()));
            Object.entries(fileStats.counts).forEach(([ext, count]) => {
              if (count > 0) {
                console.log(colors.success.bold(`     ${ext.padEnd(15)}`), chalk.white(count));
              }
            });
          }
        });
      ...
      if (showDependencies && pkg) {
        ...
        printSection('DEPENDENCIES', '📚', chalk.bgWhite.black.bold, () => {
          console.log(chalk.white.bold('  📦 Dependencies:        '), colors.primary(deps.length));
          if (deps.length > 0) {
            deps.slice(0, 5).forEach(([name, version]) => {
              console.log(chalk.gray(`     ${name.padEnd(30)} ${version}`));
            });
            if (deps.length > 5) console.log(chalk.gray(`     ... and ${deps.length - 5} more`));
          }
          console.log();
          console.log(chalk.white.bold('  🔧 Dev Dependencies:    '), colors.primary(devDeps.length));
          if (devDeps.length > 0) {
            devDeps.slice(0, 5).forEach(([name, version]) => {
              console.log(chalk.gray(`     ${name.padEnd(30)} ${version}`));
            });
            if (devDeps.length > 5) console.log(chalk.gray(`     ... and ${devDeps.length - 5} more`));
          }
        });
      }
      ...
      if (showSecurityAudit && securityAudit && securityAudit.total > 0) {
        printSection('SECURITY AUDIT', '🔒', chalk.bgRed.white.bold, () => {
          ...
          console.log(colors.error.bold('  ⚠️  Total Vulnerabilities: '), chalk.white(securityAudit.total));
          if (vulns.critical) console.log(chalk.red.bold('     Critical:  '), chalk.white(vulns.critical));
          if (vulns.high) console.log(chalk.red.bold('     High:      '), chalk.white(vulns.high));
          if (vulns.moderate) console.log(chalk.yellow.bold('     Moderate:  '), chalk.white(vulns.moderate));
          if (vulns.low) console.log(chalk.blue.bold('     Low:       '), chalk.white(vulns.low));
      ...
      if (showPerformance) {
        ...
        printSection('PERFORMANCE', '⚡', chalk.bgCyan.black.bold, () => {
          console.log(colors.primary.bold('  🚀 Startup Time:        '), chalk.white(`${loadTime}ms`));
          console.log(colors.primary.bold('  📊 Memory Usage:        '), 
            chalk.white(formatBytes(process.memoryUsage().heapUsed)));
          console.log(colors.primary.bold('  🔄 Event Loop Lag:      '), chalk.white('0ms'));
        });
      }

      // Environment Variables
      if (showEnvironmentVariables && Object.keys(envVars).length > 0) {
        printSection('ENVIRONMENT VARIABLES', '🔐', chalk.bgBlue.black.bold, () => {
      ...
      if (customMessages.length > 0) {
        printSection('CUSTOM MESSAGES', '💬', chalk.bgCyan.black.bold, () => {
          customMessages.forEach(msg => console.log(colors.primary('  ➜'), chalk.white(msg)));
        });
      }
      ...
      printSection('KEYBOARD SHORTCUTS', '⌨️', chalk.bgGray.white.bold, () => {
        ...
        shortcuts.forEach(([key, desc]) => {
          console.log(chalk.gray('  Press'), chalk.white.bold(key), chalk.gray('+ Enter to'), chalk.white(desc));
        });
      });
      ...
      console.log(colors.success(readyBorder.side) + 
        colors.success.bold('  ✓ Server is ready and watching for changes...') + 
        ' '.repeat(20) + 
      ...
      server.watcher.on('change', () => {
        ...
        if (showHotReloadStats) {
          console.log(colors.primary('  🔥 HMR Update:'), 
    ...
    buildEnd() {
      ...
      console.log(chalk.gray('  ⏱️  Build Time:'), chalk.white(`${buildTime}ms`));
      console.log(chalk.gray('  🕒 Completed at:'), chalk.white(timestamp));
      
      if (showBuildAnalytics) {
        console.log(chalk.gray('  📊 Total Builds:'), chalk.white(performanceMonitor.buildCount));
      }
      console.log();
    ...
    closeBundle() {
      ...
      console.log(chalk.gray('     Uptime:      '), chalk.white(performanceMonitor.getUptime()));
      console.log(chalk.gray('     HMR Updates: '), chalk.white(performanceMonitor.hotReloadCount));
      console.log(chalk.gray('     Builds:      '), chalk.white(performanceMonitor.buildCount));
    ...
    handleHotUpdate({ file, timestamp }) {
      if (showHotReloadStats) {
        ...
        console.log(colors.primary('  🔥 HMR:'), 
          chalk.cyan(fileName),
          chalk.gray(`(${fileExt})`),
          chalk.gray(`at ${time}`));
      }
