...
export default function RobotHead({ cursor, isHovered }) {
  ...
  useEffect(() => {
    ...
    const blinkLoop = () => {
      ...
      const blinkIntensity = emotion === 'sleepy' ? 0.3 : emotion === 'surprised' ? 0.5 : 0.1
      setBlinkState(blinkIntensity)
      setTimeout(() => setBlinkState(1), CONFIG.eyes.blinkDuration)
      const nextBlink =
        Math.random() * (CONFIG.eyes.blinkInterval[1] - CONFIG.eyes.blinkInterval[0]) +
  ...
  useEffect(() => {
    ...
    const lookAround = () => {
      ...
      if (Math.random() > 0.4) {
        ...
        setTimeout(() => setIsLookingAround(false), 1500 + Math.random() * 1000)
      }
      setTimeout(lookAround, CONFIG.eyes.lookAroundInterval + Math.random() * 2000)
    }
    lookAround()
    return () => { mounted = false }
  }, [])
  ...
  useEffect(() => {
    ...
    const cycle = setInterval(() => {
      ...
      if (nextEmotion === 'happy' || nextEmotion === 'excited' || nextEmotion === 'love') {
        setIsNodding(true)
        setIsBouncing(true)
        setTimeout(() => {
          setIsNodding(false)
  ...
  useFrame((state) => {
    ...
    // === HEAD ANIMATIONS ===
    const breathingAmp = CONFIG.head.idleAmp * emotionData.energyLevel * hoverBoost
    ...
    const secondaryBreath = Math.sin(t * breathingSpeed * 0.5) * breathingAmp * 0.3
    headRef.current.position.y = primaryBreath + secondaryBreath

    if (isBouncing) {
      headRef.current.position.y += Math.abs(Math.sin(t * 6)) * CONFIG.head.bounceAmp
    }

    const targetTilt = emotionData.headTilt * (isHovered ? 1.2 : 1)
    headRef.current.rotation.z = THREE.MathUtils.lerp(headRef.current.rotation.z, targetTilt, 0.06)

    if (isNodding) {
      headRef.current.rotation.x += Math.sin(t * 7) * 0.35 * Math.cos(t * 3) * 0.5
    }

    if (isShaking) {
      headRef.current.rotation.y += Math.sin(t * 12) * 0.18 * Math.cos(t * 5) * 0.3
    }

    if (!isNodding && !isShaking) {
      headRef.current.position.x = Math.sin(t * 0.4) * 0.02 + Math.cos(t * 0.7) * 0.015
    ...
    if (!isNodding && !isShaking) {
      ...
      headRef.current.rotation.y = THREE.MathUtils.lerp(headRef.current.rotation.y, tx, lerpSpeed)
      headRef.current.rotation.x = THREE.MathUtils.lerp(headRef.current.rotation.x, ty, lerpSpeed)
    }
    ...
    eyes.forEach(({ mesh: eye, pupil }, i) => {
      ...
      const eyeX = THREE.MathUtils.clamp(cursorTarget.x * 0.25, -CONFIG.eyes.xRange, CONFIG.eyes.xRange)
      const eyeY = THREE.MathUtils.clamp(-cursorTarget.y * 0.18, -CONFIG.eyes.yRange, CONFIG.eyes.yRange)

      eye.position.x = THREE.MathUtils.lerp(eye.position.x, offset + eyeX, CONFIG.eyes.lerp)
      eye.position.y = THREE.MathUtils.lerp(eye.position.y, 0.2 + eyeY, CONFIG.eyes.lerp)

      const blinkScale = THREE.MathUtils.lerp(eye.scale.y, blinkState, 0.25)
      eye.scale.y = blinkScale
      eye.scale.x = THREE.MathUtils.lerp(eye.scale.x, 1 + (1 - blinkScale) * 0.1, 0.2)
      ...
      if (pupil) {
        const pupilScale = emotion === 'surprised' ? 1.3 : emotion === 'angry' ? 0.6 : emotion === 'love' ? 1.2 : 1
        pupil.scale.setScalar(THREE.MathUtils.lerp(pupil.scale.x, pupilScale, 0.05))
      }
    ...
    eyebrows.forEach((brow, i) => {
      ...
      const targetAngle = emotionData.eyebrowAngle * offset * (isHovered ? 1.2 : 1)
    ...
    if (mouthRef.current) {
      ...
      mouthRef.current.scale.x += Math.sin(t * emotionData.pulseSpeed * 2) * 0.05
    }
    ...
    if (antennaRef.current) {
      ...
      antennaRef.current.rotation.z =
        Math.sin(t * CONFIG.antenna.wobbleSpeed) * CONFIG.antenna.wobbleAmp * wobbleIntensity +
      ...
      if (emotion === 'thinking') {
        ...
        antennaRef.current.position.x = THREE.MathUtils.lerp(antennaRef.current.position.x, 0, 0.1)
        antennaRef.current.position.z = THREE.MathUtils.lerp(antennaRef.current.position.z, 0, 0.1)
        antennaRef.current.position.y = THREE.MathUtils.lerp(antennaRef.current.position.y, 0.9, 0.1)
      }
    }

    // === GLOW EFFECTS ===
    if (glowRef.current) {
      const pulseValue = 0.09 + Math.sin(t * emotionData.pulseSpeed * 2.5) * 0.06 * emotionData.energyLevel
      glowRef.current.material.opacity = pulseValue * hoverBoost
      glowRef.current.scale.setScalar(1.03 + Math.sin(t * emotionData.pulseSpeed * 1.5) * 0.015)
    }

    if (outerGlowRef.current) {
      outerGlowRef.current.material.opacity = 0.04 + Math.sin(t * emotionData.pulseSpeed) * 0.03
      outerGlowRef.current.scale.setScalar(1.08 + Math.cos(t * emotionData.pulseSpeed * 0.8) * 0.02)
    }
    ...
    energyRingRefs.current.forEach((ring, i) => {
      ...
      ring.scale.setScalar(1 + Math.sin(t * 2 + i) * 0.1 * emotionData.energyLevel)
      ring.material.opacity = (0.3 + Math.sin(t * 3 + i) * 0.2) * emotionData.energyLevel
    })

    // === SCAN LINE ===
    if (scanLineRef.current) {
      scanLineRef.current.position.y = -0.5 + ((t * 0.8) % 1.5)
      scanLineRef.current.material.opacity = 0.6 - Math.abs(scanLineRef.current.position.y) * 0.3
    }
    ...
    if (particlesRef.current && particlesRef.current.geometry) {
      ...
      if (particlesRef.current.material) {
        particlesRef.current.material.opacity = 0.25 + 0.75 * emotionData.energyLevel
        particlesRef.current.material.color.set(emotionData.eyeColor)
      }
    }

    // === PULSE INTENSITY ===
    const pulseValue = 1 + Math.sin(t * emotionData.pulseSpeed * 3) * 0.35 * emotionData.energyLevel
  ...
  return (
    <group ref={headRef}>
      ...
      <mesh ref={glowRef} scale={1.03}>
        ...
        <meshBasicMaterial color={emotionData.eyeColor} transparent opacity={0.12} side={THREE.BackSide} />
      </mesh>

      <mesh ref={outerGlowRef} scale={1.08}>
        <boxGeometry args={[1.5, 1.2, 1.2]} />
        <meshBasicMaterial color={emotionData.secondaryColor} transparent opacity={0.06} side={THREE.BackSide} />
      </mesh>
      ...
      <mesh ref={scanLineRef} position={[0, 0, 0.62]}>
        ...
        <meshBasicMaterial color={emotionData.eyeColor} transparent opacity={0.6} />
      </mesh>
      ...
      {/* Eyebrows */}
      <mesh ref={leftEyebrowRef} position={[-0.45, 0.42, 0.72]} rotation={[0, 0, 0]}>
        <boxGeometry args={[0.35, 0.06, 0.02]} />
        <meshStandardMaterial color={emotionData.secondaryColor} metalness={0.2} roughness={0.6} />
      </mesh>

      <mesh ref={rightEyebrowRef} position={[0.45, 0.42, 0.72]} rotation={[0, 0, 0]}>
        <boxGeometry args={[0.35, 0.06, 0.02]} />
        <meshStandardMaterial color={emotionData.secondaryColor} metalness={0.2} roughness={0.6} />
      </mesh>
      ...
      <group ref={antennaRef} position={[0, 0.7, 0.15]}>
        <mesh geometry={geo.antenna} position={[0, 0, 0]}>
          <meshStandardMaterial color={"#222222"} metalness={0.9} roughness={0.25} />
        </mesh>
        <mesh geometry={geo.antennaTip} position={[0, 0.33, 0]}>
          <meshStandardMaterial emissive={emotionData.eyeColor} emissiveIntensity={1.6} color={emotionData.eyeColor} />
        </mesh>
        {/* antenna signal glow */}
        <mesh position={[0, 0.46, 0]} scale={0.9}>
          <sphereGeometry args={[0.12, 12, 12]} />
          <meshBasicMaterial color={emotionData.secondaryColor} transparent opacity={0.25} />
        </mesh>
      </group>
      ...
      <group ref={mouthRef} position={[0, -0.18, 0.73]}>
        <mesh>
          ...
          <meshStandardMaterial color={emotionData.secondaryColor} emissive={emotionData.eyeColor} emissiveIntensity={0.6} transparent opacity={0.95} />
        </mesh>
      </group>
      ...
      {[0, 1, 2].map((i) => (
        ...
        >
          ...
          <meshBasicMaterial color={emotionData.eyeColor} transparent opacity={0.25} side={THREE.DoubleSide} />
        </mesh>
      ))}
