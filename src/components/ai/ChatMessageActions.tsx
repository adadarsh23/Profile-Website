  ...
  return (
    ...
    >
      ...
      {isAI && onRegenerate && !isSpeaking && (
        <button onClick={onRegenerate} title="Regenerate" className="icon-action-btn">
          <RefreshCw className="w-4 h-4" />
        </button>
      )}

      {isAI && (
        <button onClick={handleSpeak} title={isSpeaking ? 'Stop' : 'Speak'} className={cn('icon-action-btn', isSpeaking && 'text-blue-400')}>
          {isSpeaking ? <X className="w-4 h-4 animate-pulse" /> : <Volume2 className="w-4 h-4" />}
        </button>
      )}
    </div>
  );
