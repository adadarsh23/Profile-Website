...
const GradualBlur = (props) => {
  ...
  const [isChatOpen, setIsChatOpen] = useState(() => {
    ...
    return (
      document.body.getAttribute('data-ai-chat-open') === 'true' ||
      ...
      const open =
        ...
        Boolean(
          document.querySelector(
            '[data-radix-dialog-content], [data-state="open"][data-side]'
          )
        );
      setIsChatOpen(open);
    };
  ...
  const blurDivs = useMemo(() => {
    ...
    const effectiveDivCount = isMobile
      ? Math.min(config.divCount, 3)
      : config.divCount;
    ...
    for (let i = 1; i <= effectiveDivCount; i++) {
      ...
      if (config.exponential) {
        ...
        blurValue =
          0.0625 * (progress * effectiveDivCount + 1) * currentStrength;
      }
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle = {
      ...
      pointerEvents: config.hoverIntensity && shouldShow ? 'auto' : 'none',
      opacity: shouldShow ? 1 : 0,
      visibility: shouldShow ? 'visible' : 'hidden',
      transition: config.animated
        ? `opacity ${config.duration} ${config.easing}, visibility ${config.duration} ${config.easing}`
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, isVisible, isChatOpen]);
