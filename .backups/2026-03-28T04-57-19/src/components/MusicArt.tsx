...
const MusicArtwork = memo(function MusicArtwork({
  ...
  const [vinylImageSrc, setVinylImageSrc] = useState('');
  ...
  useEffect(() => {
    if ((!isHovered && !isPlaying) || vinylImageSrc) {
      return;
    }

    let isMounted = true;

    import('../assets/topvinyl.png')
      .then((module) => {
        if (isMounted) {
          setVinylImageSrc(module.default);
        }
      })
      .catch(() => {
        if (isMounted) {
  ...
  }, [isHovered, isPlaying, vinylImageSrc]);
          ...
          <div className="relative w-45 h-45 sm:w-65 sm:h-65">
            <motion.div animate={vinylControls} className="w-full h-full">
              {vinylImageSrc ? (
                ...
                <div className="h-full w-full rounded-full bg-white/10 blur-[1px]" />
              )}
            </motion.div>
          </div>
        </motion.div>
