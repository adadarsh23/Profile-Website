...
export default function RobotHead({ cursor, isHovered }) {
  ...
  useFrame((state) => {
    ...
    if (screenEffect > 0) {
      const glitchAmount = screenEffect * 0.05;
      headRef.current.position.x += (Math.random() - 0.5) * glitchAmount;
      headRef.current.position.z += (Math.random() - 0.5) * glitchAmount;
      setScreenEffect((s) => Math.max(0, s - 0.05));
    }
    ...
    const sensitivity = emotionData.energyLevel * (isHovered ? 1.4 : 0.8);
    ...
    if (antennaRef.current) {
      const wobbleIntensity = emotionData.energyLevel * (isHovered ? 1.6 : 1);
    ...
    // Side panel breathing
    const sidePanelIntensity =
      0.8 + Math.sin(t * breathingSpeed) * 0.4 * hoverBoost;
    if (leftSidePanelRef.current) {
      leftSidePanelRef.current.material.emissiveIntensity = sidePanelIntensity;
    }
    if (rightSidePanelRef.current) {
      rightSidePanelRef.current.material.emissiveIntensity = sidePanelIntensity;
    }
