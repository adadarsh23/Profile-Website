import { r as n, j as t } from './vendor_react-C8wG62CJ.js';
import { b as o } from './vendor-Grk_15WJ.js';
...
const C = ({
  ...
  n.useEffect(() => {
    const e = i.current;
    if (!e) return;
    ((c.current = o.quickSetter(e, '--x', 'px')),
      (d.current = o.quickSetter(e, '--y', 'px')));
    const { width: r, height: s } = e.getBoundingClientRect();
    ((a.current = { x: r / 2, y: s / 2 }),
      c.current(a.current.x),
      d.current(a.current.y));
  }, []);
  const v = (e, r) => {
      o.to(a.current, {
        x: e,
        ...
        ease: m,
        onUpdate: () => {
          (c.current?.(a.current.x), d.current?.(a.current.y));
        },
        overwrite: !0,
      });
    },
    y = (e) => {
      ...
      (v(e.clientX - r.left, e.clientY - r.top),
        o.to(l.current, { opacity: 0, duration: 0.25, overwrite: !0 }));
    },
    f = () => {
      o.to(l.current, { opacity: 1, duration: x, overwrite: !0 });
    },
    k = (e) => {
    ...
    w = (e) => {
      ...
      (r.style.setProperty('--mouse-x', `${e.clientX - s.left}px`),
        r.style.setProperty('--mouse-y', `${e.clientY - s.top}px`));
    };
  return t.jsxs('div', {
    ref: i,
    onPointerMove: y,
    onPointerLeave: f,
    className: `relative w-full h-full flex flex-wrap justify-center items-start gap-3 ${g}`,
    style: { '--r': `${p}px`, '--x': '50%', '--y': '50%' },
    children: [
      h.map((e, r) =>
        t.jsxs(
          'article',
