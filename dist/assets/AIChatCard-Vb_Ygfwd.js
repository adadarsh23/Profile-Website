const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      'assets/ChatInput-C6nCFo3M.js',
      'assets/vendor-C8sBI81f.js',
      'assets/utils-BtE0PGZ8.js',
      'assets/ChatMessageBubble-B_als04u.js',
    ])
) => i.map((i) => d[i]);
import {
  r as e,
...
import { c } from './utils-BtE0PGZ8.js';
import { l as d } from './index-BgDpI5-0.js';
function delay(e) {
...
const m = e.memo(({ status: e }) =>
    'idle' === e || 'error' === e
      ...
      : t.jsx(a.div, {
          ...
          children: t.jsx(MessageLoading, {}),
        })
  ),
  w = e.lazy(() =>
    l(() => import('./ChatInput-C6nCFo3M.js'), __vite__mapDeps([0, 1, 2]))
  ),
  y = e.lazy(() =>
    l(
      () => import('./ChatMessageBubble-B_als04u.js'),
      __vite__mapDeps([3, 1, 2])
    )
  );
function AIChatCard({ className: l, onClose: f }) {
  const p = [
      {
        sender: 'ai',
    ...
    ],
    {
      ...
      e.useEffect(() => {
        try {
          sessionStorage.setItem(h, JSON.stringify(a));
        } catch (e) {}
      }, [a]);
      const n = e.useCallback(() => {
        try {
          (sessionStorage.removeItem(h), s([...t]));
        } catch (e) {}
      }, [t]);
      return { messages: a, setMessages: s, clearSession: n };
    })(p),
    { updateMemory: v } = (function () {
      const [t, a] = e.useState(null);
      return {
        memory: t,
        updateMemory: e.useCallback((e) => {
          if (0 === e.length) return void a(null);
          const t = e.map((e) => e.text).join(' '),
            s = t
              .toLowerCase()
              .match(/\b\w+\b/g)
              ?.filter((e) => e.length >= 4 && !u.has(e))
            ...
            n = s
              ? Object.entries(s)
                  ...
                  .map(([e]) => e)
              : [],
            i = t.length > 250 ? `...${t.slice(-250)}` : t;
          a({ summary: i, keywords: n });
        }, []),
      };
    })(),
    [j, C] = e.useState(''),
  ...
  (e.useEffect(() => {
    ...
    e.useEffect(() => {
      k.current &&
        k.current.scrollTo({ top: k.current.scrollHeight, behavior: 'smooth' });
    }, [x, N]));
  const $ = e.useCallback(
      async (e) => {
        const t = (e || j).trim();
              ...
              for (; a < 3; )
                try {
                  ...
                  if ((clearTimeout(s), !n.ok)) {
                    if (429 === n.status) {
                      (a++, await delay(1500 * a));
                      continue;
                    }
                    throw new Error(`Gemini request failed (${n.status})`);
                  }
    ...
    I = e.useCallback(() => {
      if (!x.length) return;
      const e = new Date(),
        t = `AI_Chat_${e.toISOString().replace(/[:.]/g, '-')}.txt`,
        a = x
          .map(
            (e) =>
              `[${((e) => (e ? new Date(e).toLocaleString() : 'N/A'))(e.timestamp)}] ${'user' === e.sender ? '🧑 User' : '🤖 AI'}:\n${e.text}`
          )
          .join('\n--------------------------------------\n\n'),
        s = [
          '===== AI Chat Export =====',
    ...
    _ = { hidden: { y: -20, opacity: 0 }, visible: { y: 0, opacity: 1 } };
  return t.jsxs('div', {
    className: c(
    ...
    children: [
      ...
      t.jsxs('div', {
        className:
          'relative flex flex-col w-full h-full rounded-xl border border-white/10 overflow-hidden bg-black/90 backdrop-blur-xl',
        children: [
          t.jsxs(a.div, {
            className:
