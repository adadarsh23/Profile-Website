const __vite__mapDeps = (
  i,
  m = __vite__mapDeps,
  d = m.f ||
    (m.f = [
      'assets/LogoLoop-DC5LWDF0.js',
      'assets/vendor-CLKqtzgM.js',
      'assets/CurvedLoop-DpKI00ki.js',
      'assets/index-TjyNd_uT.js',
      'assets/index-D4Pt6dGf.css',
    ])
) => i.map((i) => d[i]);
import {
  j as e,
  ac as t,
  ad as a,
  ae as s,
  af as o,
  ag as r,
  ah as d,
  ai as i,
  aj as h,
  ak as l,
  al as n,
  am as c,
  an as p,
  ao as m,
  ap as f,
  aq as x,
  ar as w,
  as as u,
  r as j,
  _ as g,
} from './vendor-CLKqtzgM.js';
const v = [
    {
      node: e.jsx(t, {}),
      title: 'Spotify',
      href: 'https://open.spotify.com/artist/7nd9x69ZcOpoft6TMDnXCa',
    },
    {
      node: e.jsx(a, {}),
      title: 'SoundCloud',
      href: 'https://soundcloud.com/adadarsh23',
    },
    {
      node: e.jsx(s, {}),
      title: 'Apple Music',
      href: 'https://music.apple.com/us/artist/%C3%A2d-adarsh/1794512299',
    },
    {
      node: e.jsx(o, {}),
      title: 'YouTube',
      href: 'https://www.youtube.com/@adadarsh23',
    },
    {
      node: e.jsx(r, {}),
      title: 'Instagram',
      href: 'https://www.instagram.com/adadarsh23/',
    },
    {
      node: e.jsx(d, {}),
      title: 'Facebook',
      href: 'https://www.facebook.com/adadarsh23',
    },
    {
      node: e.jsx(i, {}),
      title: 'LinkedIn',
      href: 'https://www.linkedin.com/in/adadarsh23/',
    },
    {
      node: e.jsx(h, {}),
      title: 'Pinterest',
      href: 'https://in.pinterest.com/adadarsh23/',
    },
    {
      node: e.jsx(l, {}),
      title: 'Reddit',
      href: 'https://www.reddit.com/user/adadarsh23/',
    },
    {
      node: e.jsx(n, {}),
      title: 'Threads',
      href: 'https://www.threads.com/adadarsh23',
    },
    {
      node: e.jsx(c, {}),
      title: 'WhatsApp',
      href: 'https://www.whatsapp.com/channel/0029VbB809Y5a23uHr5XFz0a',
    },
    { node: e.jsx(p, {}), title: 'Telegram', href: 'https://t.me/adadarsh23' },
    { node: e.jsx(m, {}), title: 'Messenger', href: 'https://m.me/adadarsh23' },
    {
      node: e.jsx(f, {}),
      title: 'Discord',
      href: 'https://discord.com/invite/h2hwYue3',
    },
    {
      node: e.jsx(x, {}),
      title: 'Twitch',
      href: 'https://www.twitch.tv/adadarsh23',
    },
    {
      node: e.jsx(w, {}),
      title: 'Snapchat',
      href: 'https://www.snapchat.com/@adadarsh2.3',
    },
    {
      node: e.jsx(u, {}),
      title: 'Gamer',
      href: 'https://adadarsh23.github.io/Portfolio/game.html',
    },
  ],
  _ = j.lazy(() =>
    g(() => import('./LogoLoop-DC5LWDF0.js'), __vite__mapDeps([0, 1]))
  ),
  y = j.lazy(() =>
    g(() => import('./CurvedLoop-DpKI00ki.js'), __vite__mapDeps([2, 1]))
  ),
  b = j.lazy(() =>
    g(
      () => import('./index-TjyNd_uT.js').then((e) => e.a),
      __vite__mapDeps([3, 1, 4])
    )
  );
function Loop() {
  const t = 80,
    a = 150,
    s = j.useMemo(() => {
      const shuffle = (e) => {
        for (let t = e.length - 1; t > 0; t--) {
          const a = Math.floor(Math.random() * (t + 1));
          [e[t], e[a]] = [e[a], e[t]];
        }
        return e;
      };
      return Array.from({ length: 4 }).map(() => {
        return {
          speed:
            ((e = t), (s = a), Math.floor(Math.random() * (s - e + 1)) + e),
          direction: Math.random() > 0.5 ? 'left' : 'right',
          logos: shuffle([...v]),
        };
        var e, s;
      });
    }, []);
  return e.jsx('section', {
    className: 'bg-black text-white mb-30 flex flex-col space-y-10',
    children: e.jsxs(j.Suspense, {
      fallback: e.jsx('div', { children: e.jsx(b, {}) }),
      children: [
        e.jsx('div', {
          style: { width: '100%' },
          className: 'flex flex-col space-y-10',
          children: s.map((t, a) =>
            e.jsx(
              'div',
              {
                style: {
                  height: '60px',
                  position: 'relative',
                  overflow: 'hidden',
                },
                className:
                  'relative overflow-hidden h-24 sm:h-32 md:h-48 w-full',
                children: e.jsx(_, {
                  logos: t.logos,
                  speed: t.speed,
                  direction: t.direction,
                  logoHeight: 48,
                  gap: 40,
                  pauseOnHover: !0,
                  scaleOnHover: !0,
                  fadeOut: !0,
                  fadeOutColor: '#000000',
                  ariaLabel: 'My Socials and Platforms',
                }),
              },
              a
            )
          ),
        }),
        e.jsx('div', {
          className: 'w-full flex flex-col gap-12',
          children: e.jsx(y, {
            marqueeText:
              'Be ✦ A Gamer Who Understands Strategy ✦ A Coder Who Solves Problems With Clarity ✦ A Producer Who Crafts Detailed Soundscapes ✦ A Creator Turning Ideas Into Functional Reality ✦',
            speed: 3,
            curveAmount: 300,
            direction: 'right',
            interactive: !0,
            className:
              'text-9xl sm:text-7xl md:text-9xl lg:text-9xl font-semibold leading-none tracking-tight select-none',
          }),
        }),
      ],
    }),
  });
}
export { Loop as default };
