import { r as l, j as t, P as b, H as f } from './vendor_react-C8wG62CJ.js';
import { Z as v } from './vendor-Grk_15WJ.js';
import './vendor_react-dom-DKAsGG5-.js';
const k = l.memo(function ({ items: r = [], baseRadius: h = 220 }) {
  const [u, p] = l.useState(window.innerWidth),
    [d, x] = l.useState(null);
  l.useEffect(() => {
  ...
  const s = l.useMemo(() => Math.min(Math.max(u / 5, 120), h * 1.6), [u, h]),
    i = u < 480 || r.length > 10,
    w = l.useMemo(() => {
      if (i) return [];
      const e = r.length;
      return r.map((n, a) => {
        const o = (a / e) * 2 * Math.PI,
          c = s + s * Math.cos(o) - 80,
          m = s + s * Math.sin(o) - 90;
        return { x: c, y: m };
      });
    ...
    g = (e, n) => {
      if (d === e) {
        const a = document.getElementById(`audio-${e}`);
        (a && a.pause(), x(null));
      } else {
        if (d !== null) {
          const o = document.getElementById(`audio-${d}`);
          o && o.pause();
        }
        const a = document.getElementById(`audio-${e}`);
        a && (a.play(), x(e));
  ...
  return t.jsxs('div', {
    className: `relative mx-auto ${i ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6' : ''}`,
    style: i ? {} : { width: s * 2, height: s * 2 },
    children: [
      r.length === 0 &&
        t.jsx('div', {
                ...
                children: [
                  t.jsxs('div', {
                    ...
                    children: [
                      ...
                      t.jsx('button', {
                        onClick: () => g(e.id, e.url),
                        className:
                          'absolute bottom-3 right-3 p-2 bg-white/20 backdrop-blur-md rounded-full hover:bg-white/40 transition',
                        'aria-label': c ? 'Pause preview' : 'Play preview',
                        children: c
                          ? t.jsx(b, { className: 'w-4 h-4 text-white' })
                          : t.jsx(f, { className: 'w-4 h-4 text-white' }),
                      }),
                  ...
                  t.jsx('div', {
                    ...
                    children: t.jsx('a', {
                      ...
                      download: e.price === 'Free' ? !0 : void 0,
                      className: `block text-center focus:outline-none focus:ring-2 focus:ring-white/40 ${e.price === 'Free' ? 'bg-white text-black hover:bg-gray-200' : 'bg-gradient-to-r from-gray-800 to-black hover:from-black hover:to-gray-900 text-white'} font-medium text-xs md:text-sm py-2 px-4 rounded-lg transition-all duration-200 shadow-md hover:shadow-xl`,
                      children:
                        e.price === 'Free'
                          ? '⬇ Download'
                          : `💳 Buy ${e.price}`,
