import { r as i, g as E, j as D } from './vendor_react-C8wG62CJ.js';
import { b as u } from './vendor-Grk_15WJ.js';
import { c as L } from './utils-x720GUhr.js';
import './vendor_react-dom-DKAsGG5-.js';
const F = ({ size: o = 40, strong: b = !1, className: m, ...h }) => {
    ...
    const w = () => {
        t.current &&
          u.to(t.current, {
            ...
            ease: 'power2.out',
            onComplete: () => {
              u.to(t.current, { scale: 1, duration: 0.4, ease: 'bounce.out' });
            },
          });
      },
      l = (r) => {
        if (!t.current) return;
        const n = r.clientX - s.current.x,
          a = r.clientY - s.current.y,
          k = Math.sqrt(n * n + a * a);
        let e = Math.atan2(a, n) * (180 / Math.PI) - c.current;
        (e > 180 && (e -= 360), e < -180 && (e += 360));
        const d = c.current + e * 0.2,
          p = Math.min(k / 30, 1.2),
          g = Math.abs(n),
          y = Math.abs(a),
          S = g + y || 1,
        ...
        (u.to(t.current, {
          ...
          (c.current = d),
          (s.current = { x: r.clientX, y: r.clientY }));
      };
    return (
...
`,
  ...
  return D.jsx('div', {
    ...
    style: { height: o, width: o, ...(b ? v : f) },
  });
};
export { F as LiquidCursor };
